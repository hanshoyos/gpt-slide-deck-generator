...
const baseStyles = {
  titleSlide: {
    title: {
      ...
      margin: [0.05, 0.1, 0.05, 0.1], wrap: true, fit: false // fit: false for better control
    },
    subtitle: {
  ...
  contentSlide: {
    title: {
      ...
      margin: [0.05, 0.1, 0.05, 0.1], wrap: true, fit: false // fit: false for titles
    },
    content: {
...
// Dynamic font size helper for slide titles
...
app.post("/createSlideDeck", async (req, res) => {
  try {
    ...
    slides.forEach((slide, idx) => {
      ...
      if (idx === 0) {
        // Title slide: dynamically size title font
        ...
        if (slide.content) pptSlide.addText(slide.content, baseStyles.titleSlide.subtitle);
      } else {
        // Content slide: dynamically size title font
        ...
        if (slide.content) pptSlide.addText(slide.content, baseStyles.contentSlide.content);
      }
